...
template <typename T> struct node_impl;
...
template <typename T>
using weak_node =
    weak_ptr<node_impl<T>>; // weak_node<T> ~~ weak_ptr<node_impl<T>>
...
public:
  ...
  // Destructor --> NO ES NECESARIO porque la memoria es administrada por el
  // puntero inteligente
  ...
  // BFS
  template <typename UnaryFunction> void recorrido_a_nivel(UnaryFunction func) {
    // un queue
    queue<node<T>> q;
    // paso # 1
    q.push(root);
  ...
  bool is_max_heap() {
    ...
    while (!q.empty()) {
      ...
      q.pop();
      if (actual->left) {
        if (actual->data < actual->left->data)
          return false;
        q.push(actual->left);
      }
      if (actual->right) {
        if (actual->data < actual->right->data)
          return false;
        q.push(actual->right);
      }
    }
